...
const Billing = ({ tableId, acPercentage }) => {
  ...
  //           // pricePerBrandCategory: orderItem[`pricePer${orderItem.barCategory}`], // Include pricePerBrandCategory
...
<script>
  ...
  // const handleCheckboxChange = (itemName) => {
  //   // Check if the selected menu is already in the currentOrder array
  //   const alreadySelected = currentOrder.find((item) => item.name === itemName);

  //   if (!alreadySelected) {
  //     // Find the selected menu in the bar menus array
  //     const selectedMenuFromBar = barMenus.find((menu) => menu.name === itemName);

  //     // Find the selected menu in the regular menus array if not found in the bar menus
  //     const selectedMenuFromRegular = regularMenus.find((menu) => menu.name === itemName);

  //     if (selectedMenuFromBar) {
  //       // Add the selected menu from bar menus as a new item in the current order
  //       setCurrentOrder((prevOrder) => [...prevOrder, { ...selectedMenuFromBar, quantity: 1 }]);
  //     } else if (selectedMenuFromRegular) {
  //       // Add the selected menu from regular menus as a new item in the current order
  //       setCurrentOrder((prevOrder) => [...prevOrder, { ...selectedMenuFromRegular, quantity: 1 }]);
  //     }
  //   } else {
  //     // If the menu is already in the order, you might want to update its quantity or handle it differently
  //     // For example, you could increase its quantity here
  //     setCurrentOrder((prevOrder) =>
  //       prevOrder.map((item) =>
  //         item.name === itemName
  //           ? { ...item, quantity: item.quantity + 1 }
      ...
      <div className="container mx-auto">
        <div className="flex lg:flex-row shadow-lg">
          ...
          <div className=" w-56 lg:w-3/5 md:w-96 hidden md:block bg-white min-h-screen">
            ...
            {showCategoryMenus && (
              <div
                className="cursor-pointer grid grid-cols-2 bg-white md:grid-cols-3 lg:grid-cols-4 gap-3 lg:px-3 md:px-2 px-2 mt-3 custom-sidescrollbars overflow-scroll lg:max-h-[calc(86vh-8rem)] md:max-h-[calc(84vh-1rem)] max-h-[calc(97vh-1rem)] sm:max-h-[calc(80vh-1rem)]"
              >
                {(menus.menus || menus)
                  .filter(filterMenus) // Apply the filterMenus function
                  .map((product, index) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="justify-center flex">
                          <img
                            src={product.imageUrl ? `http://localhost:5000/${product.imageUrl}` : `/tray.png`}
                            className={`object-cover rounded-md ${product.imageUrl ? 'lg:w-24 lg:h-16 md:w-14 md:h-10 w-8 h-8 lg:mt-1 -mt-4 md:mt-1' : 'lg:w-12 lg:h-10 md:w-7 md:h-7 w-8 h-8 lg:mt-6 mt-2 ml-4 md:mt-4'} hidden lg:block`}
                            alt=""
                          />
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-4 gap-4 p-4" key={selectedBarMenuItem._id}>
                  ...
                  {selectedBarMenuItem.childMenus.map((childMenu) => {
                    if (childMenu.barCategory && childMenu.pricePer[`pricePer${childMenu.barCategory}`] !== 0) {
                      return (
                        <div key={childMenu._id} className="bg-yellow-400 p-2 rounded-md text-gray-900" onClick={() => addToOrder(childMenu)}>
                          <p className="text-center font-semibold text-sm">{selectedBarMenuItem.name}</p>
                          <p className="text-center font-semibold">{childMenu.barCategory}</p>
                          {/* Render the price dynamically */}
                          <p className="text-center">₹{childMenu.pricePer[`pricePer${childMenu.barCategory}`]}</p>
                        </div>
                      );
                    }
                    return null;
